...
def test_write_deduplicates_by_lesson():
    ...
    # Same lesson text should not create a second row; counts go up.
...
def test_update_fitness_improves_on_success():
    ...
    # 1 success, 1 failure -> fitness 0.0
    assert updated.fitness == 0.0
    updated = mem.update_fitness(entry.memory_id, success=True)
    # 2 success, 1 failure -> fitness 1/3
...
def test_prune_removes_low_fitness_after_min_uses():
    ...
    # 1 failure so far, need 3 uses to consider pruning.
    mem.update_fitness(entry.memory_id, False)
    mem.update_fitness(entry.memory_id, False)
    # Now 3 failures total, fitness = -1.0, should be pruned.
    pruned = mem.prune()
    assert len(pruned) == 1
